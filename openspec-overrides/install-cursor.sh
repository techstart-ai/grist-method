#!/usr/bin/env bash
# Install GRIST OpenSpec overlay into a project using Cursor.
#
# Usage: ./install-cursor.sh <project-root> [--dry-run] [--uninstall]
#
# This installer targets projects where OpenSpec commands run as Cursor skills
# (.cursor/skills/opsx-*/). It mirrors install-claude-code.sh with path
# adjustments for Cursor's directory layout.
#
# What it does:
...
#   5. Appends GRIST always-on rules to AGENTS.md (idempotent)
#   6. Creates .grist/context-pack.md from template if not present
...
usage() {
  ...
  echo
  echo "  Installs GRIST OpenSpec overlay for Cursor projects."
  echo "  Requires Cursor skills (.cursor/skills/opsx-*)."
  echo
  echo "Options:"
