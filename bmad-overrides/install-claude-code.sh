#!/usr/bin/env bash
# Install GRIST overlays into a BMAD project that uses Claude Code skills.
#
# Usage: ./install-claude-code.sh <project-root>
#
# This installer targets projects where BMAD runs via Claude Code skills
# (.claude/skills/bmad-*/workflow.md) rather than the BMAD npm/framework
# TOML config layer. If your project uses the BMAD CLI with TOML config,
...
#   3. Installs the grist skill and slash command
#   4. Appends always-on rules to CLAUDE.md (idempotent)
...
usage() {
  ...
  echo
  echo "  Installs GRIST overlays for BMAD projects using Claude Code skills."
  ...
  echo "  --uninstall   Remove GRIST injection blocks and CLAUDE.md rules"
  exit 2
}
...
if [[ ! -d "$SKILLS_DIR" ]]; then
  log_err "$PROJECT_ROOT does not have .claude/skills/ — is this a Claude Code project?"
  exit 1
fi
...
if $UNINSTALL; then
  ...
  # Remove CLAUDE.md rules
  if [[ -f "$PROJECT_ROOT/CLAUDE.md" ]]; then
    remove_grist_blocks "$PROJECT_ROOT/CLAUDE.md"
  fi
...
# Uses sed + temp files to avoid awk -v multiline content bugs.
inject_block() {
  local target_file="$1"
...
# --- Step 3: Install grist skill and command --------------------------------
...
if $DRY_RUN; then
  ...
  # Also copy TOML for legacy compatibility
...
if [[ -f "$CLAUDE_MD" ]] && grep -q 'GRIST:RULES' "$CLAUDE_MD"; then
  log_skip "CLAUDE.md already contains GRIST rules (marker found)"
...
# Claude Code skills (.claude/skills/bmad-*/) ignore this file.
# GRIST emission is wired directly into workflow step files by install-claude-code.sh.
...
for tool in bmad-create-prd bmad-create-architecture bmad-create-story bmad-dev-story bmad-code-review; do
  ...
  if [[ -f "$toml_file" ]]; then
    if grep -q "Claude Code skills" "$toml_file"; then
...
echo
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
printf "${GREEN}GRIST installed for Claude Code at:${NC} %s\n" "$PROJECT_ROOT"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo
...
echo "  • Always-on rules in CLAUDE.md apply even without /grist activation."
echo
echo "Next steps:"
