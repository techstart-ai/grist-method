#!/usr/bin/env bash
# Install GRIST overlays into a BMAD project that uses Cursor skills.
#
# Usage: ./install-cursor.sh <project-root> [--dry-run] [--uninstall]
#
# This installer targets projects where BMAD runs via Cursor skills
# (.cursor/skills/bmad-*/workflow.md). It mirrors install-claude-code.sh
# with path adjustments for Cursor's directory layout.
#
# What it does:
...
#   3. Installs the grist skill (skill = command in Cursor)
#   4. Installs always-on .mdc rule to .cursor/rules/
#   5. Appends always-on rules to AGENTS.md (idempotent)
#   6. Creates .grist/context-pack.md from template
#
...
usage() {
  ...
  echo
  echo "  Installs GRIST overlays for BMAD projects using Cursor skills."
...
if [[ ! -d "$SKILLS_DIR" ]]; then
  log_err "$PROJECT_ROOT does not have .cursor/skills/ — is this a Cursor project?"
  exit 1
fi
...
if $UNINSTALL; then
  ...
  # Remove .cursor/rules/grist.mdc
  if [[ -f "$PROJECT_ROOT/.cursor/rules/grist.mdc" ]]; then
    if $DRY_RUN; then
      log_info "[dry-run] Would remove .cursor/rules/grist.mdc"
    else
      rm -f "$PROJECT_ROOT/.cursor/rules/grist.mdc"
      log_ok "Removed .cursor/rules/grist.mdc"
    fi
  fi
...
echo
log_info "Step 3: Installing grist skill (skill = command in Cursor)"

# Skill
SKILL_DST="$SKILLS_DIR/grist"
mkdir -p "$SKILL_DST"
if $DRY_RUN; then
  log_info "[dry-run] Would install .cursor/skills/grist/SKILL.md"
else
  cp "$GRIST_ROOT/skills/grist/SKILL.md" "$SKILL_DST/SKILL.md"
  log_ok ".cursor/skills/grist/SKILL.md"
fi

# --- Step 4: Install .cursor/rules/grist.mdc + AGENTS.md rules -------------

echo
log_info "Step 4: Installing always-on rules"

# .cursor/rules/grist.mdc
RULES_DIR="$PROJECT_ROOT/.cursor/rules"
mkdir -p "$RULES_DIR"
if $DRY_RUN; then
  log_info "[dry-run] Would install .cursor/rules/grist.mdc"
else
  cp "$GRIST_ROOT/cursor-rules/grist.mdc" "$RULES_DIR/grist.mdc"
  log_ok ".cursor/rules/grist.mdc"
fi
...
# Cursor skills (.cursor/skills/bmad-*/) ignore this file.
# GRIST emission is wired directly into workflow step files by install-cursor.sh.
...
for tool in bmad-create-prd bmad-create-architecture bmad-create-story bmad-dev-story bmad-code-review; do
  ...
  if [[ -f "$toml_file" ]]; then
    if grep -q "Cursor skills" "$toml_file"; then
...
echo
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
printf "${GREEN}GRIST installed for Cursor at:${NC} %s\n" "$PROJECT_ROOT"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo
echo "What happens now:"
echo "  • Invoke the grist skill in Cursor chat to activate a mode."
...
echo "  • Always-on rules in AGENTS.md + .cursor/rules/grist.mdc apply automatically."
echo
echo "Next steps:"
